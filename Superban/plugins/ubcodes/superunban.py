...
async def super_unban(_, message):
    ...
    superunban_msg = await message.edit_text(
    ...
    superunban_request_messages[user.id] = superunban_msg

    try:
        await superunban_msg.pin(disable_notification=True)
    except Exception as e:
...
async def handle_super_unban_callback(client: Client, query: CallbackQuery):
    ...
    try:
        if action == "approve":
            ...
            if user.id in superunban_request_messages:
                try:
                    await superunban_request_messages[user.id].edit(
            ...
            try:
                ...
                if user.id in superunban_request_messages:
                    try:
                        await superunban_request_messages[user.id].edit(complete_text)
                    except Exception as e:
                        logging.warning(f"Failed to edit superunban_request_messages to complete: {e}")
